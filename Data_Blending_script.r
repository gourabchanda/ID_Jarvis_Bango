...
psales.bango.df1%<>%convert(num(`Gross Sales Value (GSV)`:Conversion))
...
ssales.bango.df1%<>%convert(num(`Gross Sec Sales (TUR)`:Conversion))
#calculate the derived KPI's from the exisiting KPI's using the conversion factor and add the columns into the data frame
...
sstock.bango.df1%<>%convert(num(`Secondary Stock Volume`:Conversion))
#calculate the derived KPI's from the exisiting KPI's using the conversion factor and add the columns into the data frame
...
droo.bango.df%<>%convert(num(`OriginalOrder Qty`:`Dispatched Qty`))
...
penetration.path <- paste0(folder.penetration.path,"/GD ID PENETRATION BANGO.xlsx")
penetration.cat.path <- paste0(folder.penetration.path,"/GD ID PENETRATION Total Category.xlsx")
...
consolidated.list1 <- merge.data.frame(x = consolidated.list,y = tts.bango.gt,by = primary_key,all = T)
consolidated.list2 <- merge.data.frame(x=consolidated.list1,y = bmi.bango.gt,by=primary_key,all = T)

consolidated.list2[is.na(consolidated.list2)] <- ""
total.bango.gt <- consolidated.list2
if(nrow(total.bango.gt)!=0){
...
total.bango.gt.final <- format(total.bango.gt.final,scientific = F)
  ...
  period <- unique(lubridate::dmy(paste("01-", total.bango.gt$Month , sep ="")))
  print(paste0("There are no Data available for the GT Model for the period",period))
}

writexl::write_xlsx(total.bango.gt.final,paste0(consolidated.backup.path,"/formated_total_bango_gt_",file_latest_time,".xlsx"),format_headers = T)
...
consolidated.list.mt <- merge.data.frame(consolidated.list.mt,bmi.bango.mt,by = primary_key,all = T)
consolidated.list.mt <- merge.data.frame(consolidated.list.mt,tts.bango.mt,by = primary_key,all = T)
consolidated.list.mt[is.na(consolidated.list.mt)] <- ""
total.bango.mt <- consolidated.list.mt
if(nrow(total.bango.mt)!=0){
...
total.bango.mt.final <- format(total.bango.mt.final,scientific =F)
  ...
  period <- unique(lubridate::dmy(paste("01-", total.bango.mt$Month , sep ="")))
  print(paste0("There are no Data available for the MT Model for the period",period))
}
writexl::write_xlsx(total.bango.mt.final,paste0(consolidated.backup.path,"/formated_total_bango_mt_",file_latest_time,".xlsx"),format_headers = T)
...
writexl::write_xlsx(total.final.bango,paste0(consolidated.backup.path,"/ID_total_bango_",file_latest_time,".xlsx"))
...
bmi.unmapped.region.aggregate <- bmi.unmapped.join%>%
  group_by(`Fiscal year/period`,Brand_Name,`Account/Region.y`)%>%
  summarise(`Brand & Marketing Investment`=sum(`Brand & Marketing Investment`),`Brand & Marketing Investment Trade`=sum(`Brand & Marketing Investment Trade`),`Brand & Marketing Investment Consumer`=sum(`Brand & Marketing Investment Consumer`),`Promotional Expenses`=sum(`Promotional Expenses`),`Promotion Packaging Material Cost`=sum(`Promotion Packaging Material Cost`),`Promotion Packaging Material Cost Trade`=sum(`Promotion Repacking Cost Trade`),`Promotion Packaging Material Cost Consumer`=sum(`Promotion Packaging Material Cost Consumer`),`Promotion Repacking Cost`=sum(`Promotion Repacking Cost`),`Promotion Repacking Cost Trade`=sum(`Promotion Repacking Cost Trade`),`Promotion Repacking Cost Consumer`=sum(`Promotion Repacking Cost Consumer`),`Promotion Communication Material Cost`=sum(`Promotion Communication Material Cost`),`Promotion Communication Material Cost Trade`=sum(`Promotion Communication Material Cost Trade`),`Promotion Communication Material Cost Consumer`=sum(`Promotion Communication Material Cost Consumer`),`Promo Samples, Gifts and Incentive Costs`=sum(`Promo Samples, Gifts and Incentive Costs`),`Promo Samples, Gifts and Incentive Costs Consumer`=sum(`Promo Samples, Gifts and Incentive Costs Consumer`),`Promotion Team Cost`=sum(`Promotion Team Cost`),`Promotion Team Cost Trade`=sum(`Promotion Team Cost Trade`),`Promotion Team Cost Consumer`=sum(`Promotion Team Cost Consumer`),`Promo Agency Remun Fees & Commissions`=sum(`Promo Agency Remun Fees & Commissions`),`Promo Agency Remuneration Fees & Commissions Trade`=sum(`Promo Agency Remuneration Fees & Commissions Trade`),`Promo Agency Remuneration Fees & Commissions Consumer`=sum(`Promo Agency Remuneration Fees & Commissions Consumer`))

bmi.unmapped.region.aggregate <- setnames(bmi.unmapped.region.aggregate,old = c("Brand_Name","Account/Region.y"),new = c("BRAND","Account/Region"))
#perform the mapping for the months
...
penetration_folder_path <- "C:/Users/Gourab.Chanda/OneDrive - Unilever/ID_Jarvis_Bango-HICW10NA512SG4H/Extracted_Files/Penetration/Penetration_xlsx"
penetration.bango.oi.path <- paste0(penetration_folder_path,"/GD ID PENETRATION BANGO OI Region.xlsx")
penetration.bango.region.path <-  paste0(penetration_folder_path,"/GD ID PENETRATION BANGO Region.xlsx")
penetration.totalcat.oi.path <- paste0(penetration_folder_path,"/GD ID PENETRATION Total Category OI Region.xlsx")
penetration.totalcat.region.path <- paste0(penetration_folder_path,"/GD ID PENETRATION Total Category Region.xlsx")
...
writexl::write_xlsx(bango.region.cej,paste0(consolidated.backup.path,"\\ID_Central&EastJava_Bango_",file_latest_time,".xlsx"))
writexl::write_xlsx(bango.region.oi,paste0(consolidated.backup.path,"\\ID_OtherIsland_Bango_",file_latest_time,".xlsx"))
writexl::write_xlsx(bango.region.sumatra,paste0(consolidated.backup.path,"\\ID_Sumatera_Bango_",file_latest_time,".xlsx"))
writexl::write_xlsx(bango.region.wj,paste0(consolidated.backup.path,"\\ID_WestJava_Bango_",file_latest_time,".xlsx"))
...
sstock.packgroup.df%<>%convert(num(`Secondary Stock Volume`:`Secondary Stock Value [@ DT Rate.]`,Conversion))
...
droo.packgroup.df%<>%convert(num(`OriginalOrder Qty`:`Dispatched Qty`,Conversion))
#perform the grouping and aggregate of the KPI's
...
penetration_packgroup_folder <- folder.penetration.path

file_names <- list.files(path = paste0(penetration_packgroup_folder))
#store the files data into variables with packgroup abbrevation 
penetration_bmbp_oi <- tibble::as_tibble(read_xlsx(paste0(penetration_packgroup_folder,"/GD ID PENETRATION Bango Manis Big Pouch OI.xlsx"),sheet = 1,skip = 14))
penetration_bmbp <- tibble::as_tibble(read_xlsx(paste0(penetration_packgroup_folder,"/GD ID PENETRATION Bango Manis Big Pouch.xlsx"),sheet = 1,skip = 14))
penetration_bmbs_oi <- tibble::as_tibble(read_xlsx(paste0(penetration_packgroup_folder,"/GD ID PENETRATION Bango Manis Big Sachet OI.xlsx"),sheet = 1,skip = 14))
penetration_bmbs <- tibble::as_tibble(read_xlsx(paste0(penetration_packgroup_folder,"/GD ID PENETRATION Bango Manis Big Sachet.xlsx"),sheet = 1,skip = 14))
penetration_bmmb_oi <- tibble::as_tibble(read_xlsx(paste0(penetration_packgroup_folder,"/GD ID PENETRATION Bango Manis Medium Bottle OI.xlsx"),sheet = 1,skip = 14))
penetration_bmmb <- tibble::as_tibble(read_xlsx(paste0(penetration_packgroup_folder,"/GD ID PENETRATION Bango Manis Medium Bottle.xlsx"),sheet = 1,skip = 14))
penetration_bmmp_oi <- tibble::as_tibble(read_xlsx(paste0(penetration_packgroup_folder,"/GD ID PENETRATION Bango Manis Medium Pouch OI.xlsx"),sheet = 1,skip = 14))
penetration_bmmp <- tibble::as_tibble(read_xlsx(paste0(penetration_packgroup_folder,"/GD ID PENETRATION Bango Manis Medium Pouch.xlsx"),sheet = 1,skip = 14))
penetration_bmsb_oi <- tibble::as_tibble(read_xlsx(paste0(penetration_packgroup_folder,"/GD ID PENETRATION Bango Manis Small Bottle OI.xlsx"),sheet = 1,skip = 14))
penetration_bmsb <- tibble::as_tibble(read_xlsx(paste0(penetration_packgroup_folder,"/GD ID PENETRATION Bango Manis Small Bottle.xlsx"),sheet = 1,skip = 14))
penetration_bmsp_oi <- tibble::as_tibble(read_xlsx(paste0(penetration_packgroup_folder,"/GD ID PENETRATION Bango Manis Small Pouch OI.xlsx"),sheet = 1,skip = 14))
penetration_bmsp <- tibble::as_tibble(read_xlsx(paste0(penetration_packgroup_folder,"/GD ID PENETRATION Bango Manis Small Pouch.xlsx"),sheet = 1,skip = 14))
#clean the dataframes using clean function
...
write_xlsx(packgroup.bmbp.cej,paste0(consolidated.backup.path,"/packgroup.bmbp.cej_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmbp.oi,paste0(consolidated.backup.path,"/packgroup.bmbp.oi_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmbp.sumatera,paste0(consolidated.backup.path,"/packgroup.bmbp.sumatera_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmbp.wj,paste0(consolidated.backup.path,"/packgroup.bmbp.wj_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmbs.cej,paste0(consolidated.backup.path,"/packgroup.bmbs.cej_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmbs.oi,paste0(consolidated.backup.path,"/packgroup.bmbs.oi_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmbs.sumatera,paste0(consolidated.backup.path,"/packgroup.bmbs.sumatera_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmbs.wj,paste0(consolidated.backup.path,"/packgroup.bmbs.wj_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmmp.cej,paste0(consolidated.backup.path,"/packgroup.bmmp.cej_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmmp.oi,paste0(consolidated.backup.path,"/packgroup.bmmp.oi_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmmp.sumatera,paste0(consolidated.backup.path,"/packgroup.bmmp.sumatera_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmmp.wj,paste0(consolidated.backup.path,"/packgroup.bmmp.wj_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmmb.cej,paste0(consolidated.backup.path,"/packgroup.bmmb.cej_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmmb.oi,paste0(consolidated.backup.path,"/packgroup.bmmb.oi_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmmb.sumatera,paste0(consolidated.backup.path,"/packgroup.bmmb.sumatera_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmmb.wj,paste0(consolidated.backup.path,"/packgroup.bmmb.wj_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmsb.cej,paste0(consolidated.backup.path,"/packgroup.bmsb.cej_",file_latest_time,".xlsx"))
...
#packgroup.bmsb.cej <- data.frame(lapply(packgroup.bmsb.cej, gsub, pattern='NA', replacement=''))
write_xlsx(packgroup.bmsb.oi,paste0(consolidated.backup.path,"/packgroup.bmsb.oi_",file_latest_time,".xlsx"))
...
#packgroup.bmsb.cej <- data.frame(lapply(packgroup.bmsb.cej, gsub, pattern='NA', replacement=''))
write_xlsx(packgroup.bmsb.sumatera,paste0(consolidated.backup.path,"/packgroup.bmsb.sumatera_",file_latest_time,".xlsx"))
...
#packgroup.bmsb.cej <- data.frame(lapply(packgroup.bmsb.cej, gsub, pattern='NA', replacement=''))
write_xlsx(packgroup.bmsb.wj,paste0(consolidated.backup.path,"/packgroup.bmsb.wj_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmsp.cej,paste0(consolidated.backup.path,"/packgroup.bmsp.cej_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmsp.oi,paste0(consolidated.backup.path,"/packgroup.bmsp.oi_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmsp.sumatera,paste0(consolidated.backup.path,"/packgroup.bmsp.sumatera_",file_latest_time,".xlsx"))
...
write_xlsx(packgroup.bmsp.wj,paste0(consolidated.backup.path,"/packgroup.bmsp.wj_",file_latest_time,".xlsx"))
...
sellout.mt.input.path <- "C:/Users/Gourab.Chanda/OneDrive - Unilever/ID_Jarvis_Bango-HICW10NA512SG4H/Extracted_Files/Sellout"
...
sellout.account.df%<>%convert(num(Sellout_Value,unit_price))
...
sellout.account.data <- as.numeric(sellout.account.data$Sellout_Value)
...
bmi.unmapped.mt.data.mapped <- setnames(bmi.unmapped.mt.data.mapped,old = "Account/Region.y",new = "Account/Region")
...
consolidated.list1 <- merge.data.frame(x = psales.basepack.final,droo.basepack.final,by = pkey,all = T)
consolidated.list2 <- merge.data.frame(x=consolidated.list1,y = tts.basepack.final,all = T)
consolidated.list3 <- merge.data.frame(x = consolidated.list2,y = bmi.basepack.final,all = T)
consolidated.list4 <- merge.data.frame(x=consolidated.list3,y=sellout.basepack.final,all.x = T)

#store the Base pack in a given vector
...
write_xlsx(alfamart.bkm275,paste0(consolidated.backup.path,"/alfamart.bkm275_",file_latest_time,".xlsx"))
...
write_xlsx(alfamart.bkm620,paste0(consolidated.backup.path,"/alfamart.bkm620_",file_latest_time,".xlsx"))
...
write_xlsx(alfamart.bkm220,paste0(consolidated.backup.path,"/alfamart.bkm220_",file_latest_time,".xlsx"))
...
write_xlsx(alfamart.bkmp220,paste0(consolidated.backup.path,"/alfamart.bkmp220_",file_latest_time,".xlsx"))
...
write_xlsx(alfamart.bkmp135,paste0(consolidated.backup.path,"/alfamart.bkmp135_",file_latest_time,".xlsx"))
...
write_xlsx(alfamart.bsofs60,paste0(consolidated.backup.path,"/alfamart.bsofs60_",file_latest_time,".xlsx"))
...
write_xlsx(alfamart.bsofms30,paste0(consolidated.backup.path,"/alfamart.bsofms30_",file_latest_time,".xlsx"))
...
write_xlsx(alfamart.bkm135,paste0(consolidated.backup.path,"/alfamart.bkm135_",file_latest_time,".xlsx"))
...
write_xlsx(alfamart.bsof580,paste0(consolidated.backup.path,"/alfamart.bsof580_",file_latest_time,".xlsx"))

}else {
  svDialogs::dlg_message(paste0("The Alfamart has",nrow(alfamart.list),"Observations"))
}


indomart.list <- consolidated.list4%>%
  filter(str_detect(Customer,"MT Indo"))
if(nrow(indomart.list)!=0){
...
indomart.headers <- c("Month",
                      ...
                      "Customer",
                      "Bango_GSV",
                      "Bango_Primary Sales Qty(PC)",
                      "Bango_NIV",
                      "Bango_Primary Sales Qty(Kg)",
                      "Bango_Primary Sales Qty(Tonns)",
...
write_xlsx(indomart.bkm275,paste0(consolidated.backup.path,"/indomart.bkm275_",file_latest_time,".xlsx"))

indomart.bkm620 <- indomart.final.list%>%
  filter(Product==basepack_list[2])
data.frame(lapply(indomart.bkm620, gsub, pattern=NA, replacement=''))
write_xlsx(indomart.bkm620,paste0(consolidated.backup.path,"/indomart.bkm620_",file_latest_time,".xlsx"))


indomart.bkm220 <- indomart.final.list%>%
  filter(Product==basepack_list[3])
data.frame(lapply(indomart.bkm220, gsub, pattern=NA, replacement=''))
write_xlsx(indomart.bkm220,paste0(consolidated.backup.path,"/indomart.bkm220_",file_latest_time,".xlsx"))

indomart.bkmp220 <- indomart.final.list%>%
  filter(Product==basepack_list[4])
data.frame(lapply(indomart.bkmp220, gsub, pattern=NA, replacement=''))
write_xlsx(indomart.bkmp220,paste0(consolidated.backup.path,"/indomart.bkmp220_",file_latest_time,".xlsx"))


indomart.bkmp135 <- indomart.final.list%>%
  filter(Product==basepack_list[5])
data.frame(lapply(indomart.bkmp135, gsub, pattern=NA, replacement=''))
write_xlsx(indomart.bkmp135,paste0(consolidated.backup.path,"/indomart.bkmp135_",file_latest_time,".xlsx"))

indomart.bsofs60 <- indomart.final.list%>%
  filter(Product==basepack_list[6])
data.frame(lapply(indomart.bsofs60, gsub, pattern=NA, replacement=''))
write_xlsx(indomart.bsofs60,paste0(consolidated.backup.path,"/indomart.bsofs60_",file_latest_time,".xlsx"))

indomart.bsofms30 <- indomart.final.list%>%
  filter(Product==basepack_list[7])
data.frame(lapply(indomart.bsofms30, gsub, pattern=NA, replacement=''))
write_xlsx(indomart.bsofms30,paste0(consolidated.backup.path,"/indomart.bsofms30_",file_latest_time,".xlsx"))

indomart.bkm135 <- indomart.final.list%>%
  filter(Product==basepack_list[8])
data.frame(lapply(indomart.bkm135, gsub, pattern=NA, replacement=''))
write_xlsx(indomart.bkm135,paste0(consolidated.backup.path,"/indomart.bkm135_",file_latest_time,".xlsx"))


indomart.bsof580 <- indomart.final.list%>%
  filter(Product==basepack_list[9])
data.frame(lapply(indomart.bsof580, gsub, pattern=NA, replacement=''))
write_xlsx(indomart.bsof580,paste0(consolidated.backup.path,"/indomart.bsof580_",file_latest_time,".xlsx"))


}else {
  svDialogs::dlg_message(paste0("The Indomart has",nrow(alfamart.list),"Observations"))
}


Carrefour.list <- consolidated.list4%>%
  filter(str_detect(Customer,"MT Carrefour"))
if(nrow(Carrefour.list!=0)){
...
write_xlsx(Carrefour.bkm275,paste0(consolidated.backup.path,"/Carrefour.bkm275_",file_latest_time,".xlsx"))


Carrefour.bkm620 <- Carrefour.final.list%>%
  filter(Product==basepack_list[2])
data.frame(lapply(Carrefour.bkm620, gsub, pattern=NA, replacement=''))
write_xlsx(Carrefour.bkm620,paste0(consolidated.backup.path,"/Carrefour.bkm620_",file_latest_time,".xlsx"))


Carrefour.bkm220 <- Carrefour.final.list%>%
  filter(Product==basepack_list[3])
data.frame(lapply(Carrefour.bkm220, gsub, pattern=NA, replacement=''))
write_xlsx(Carrefour.bkm220,paste0(consolidated.backup.path,"/Carrefour.bkm220_",file_latest_time,".xlsx"))


Carrefour.bkmp220 <- Carrefour.final.list%>%
  filter(Product==basepack_list[4])
data.frame(lapply(Carrefour.bkmp220, gsub, pattern=NA, replacement=''))
write_xlsx(Carrefour.bkmp220,paste0(consolidated.backup.path,"/Carrefour.bkmp220_",file_latest_time,".xlsx"))


Carrefour.bkmp135 <- Carrefour.final.list%>%
  filter(Product==basepack_list[5])
data.frame(lapply(Carrefour.bkmp135, gsub, pattern=NA, replacement=''))
write_xlsx(Carrefour.bkmp135,paste0(consolidated.backup.path,"/Carrefour.bkmp135_",file_latest_time,".xlsx"))


Carrefour.bsofs60 <- Carrefour.final.list%>%
  filter(Product==basepack_list[6])
data.frame(lapply(Carrefour.bsofs60, gsub, pattern=NA, replacement=''))
write_xlsx(Carrefour.bsofs60,paste0(consolidated.backup.path,"/Carrefour.bsofs60_",file_latest_time,".xlsx"))


Carrefour.bsofms30 <- Carrefour.final.list%>%
  filter(Product==basepack_list[7])
data.frame(lapply(Carrefour.bsofms30, gsub, pattern=NA, replacement=''))
write_xlsx(Carrefour.bsofms30,paste0(consolidated.backup.path,"/Carrefour.bsofms30_",file_latest_time,".xlsx"))


Carrefour.bkm135 <- Carrefour.final.list%>%
  filter(Product==basepack_list[8])
data.frame(lapply(Carrefour.bkm135, gsub, pattern=NA, replacement=''))
write_xlsx(Carrefour.bkm135,paste0(consolidated.backup.path,"/Carrefour.bkm135_",file_latest_time,".xlsx"))


Carrefour.bsof580 <- Carrefour.final.list%>%
  filter(Product==basepack_list[9])
data.frame(lapply(Carrefour.bsof580, gsub, pattern=NA, replacement=''))
write_xlsx(Carrefour.bsof580,paste0(consolidated.backup.path,"/Carrefour.bsof580_",file_latest_time,".xlsx"))
}else {
  svDialogs::dlg_message(paste0("The Indomart has",nrow(Carrefour.list)," " ,"Observations"))
}
...
bmi.unmapped.sch.data.mapped <- setnames(bmi.unmapped.sch.data.mapped,old ="Sub channel.y" ,new = "Sub channel")
...
colnames(sch.final.data) <- sch.column.headers 
  ...
  svDialogs::dlg_message(paste0("The subchannel has",nrow(sch.final.data),"Observations"))
}
...
writexl::write_xlsx(HSM.final.data,paste0(consolidated.backup.path,"/HSM.final.data_",file_latest_time,".xlsx"))

Mini.final.data <- sch.final.data%>%
  filter(Customer==sub_channel.list[2])
Mini.final.data <- format(Mini.final.data,scientific=F)

writexl::write_xlsx(Mini.final.data,paste0(consolidated.backup.path,"/Mini.final.data_",file_latest_time,".xlsx"))
...
bmi.unmapped.account.data.mapped <- setnames(bmi.unmapped.account.data.mapped,old = "Account/Region.y" ,new ="Account/Region" )
#filter out the observation for which the sub channel is {HSM.Mini} and convert the KPI's from Char to num
...
colnames(account.final.data) <- account.col.headers
  ...
  svDialogs::dlg_message(paste0("The subchannel has",nrow(account.final.data),"Observations"))
}
...
writexl::write_xlsx(account.alfamart.data,paste0(consolidated.backup.path,"/MT.alfamart_",file_latest_time,".xlsx"))
writexl::write_xlsx(account.indomart.data,paste0(consolidated.backup.path,"/MT.Indomart_",file_latest_time,".xlsx"))
writexl::write_xlsx(account.carrefour.data,paste0(consolidated.backup.path,"/MT.Carrefour_",file_latest_time,".xlsx"))

end_time <- Sys.time()
script_time <- round(end_time-start_time,2)
print(paste0("The Data Blending Time is :", script_time))