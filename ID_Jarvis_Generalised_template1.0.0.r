###### total Bango GT Model Script (Internal Data Source) KPI calculation and Consolidation######
###### There are total 8 internal Data Source - Primary Sales,Secondary Sales,Secondary Stock,DRO,Penetration,TTS,BMI,Itrust
###### The base KPI list for Primary Sales {GSV,NIV,Sales Quantity(In PCS)},Secondary Sales{Sec sales value,Sec sales Qty(PC)}
###### The base KPI list for Secondary Stock {Sec stock value,Sec stock Qty(PC)},DROO {Orginal order qty,Final expected Order Qty,Dispatch qty}
###### The base KPI list for Penetration {Penetration_Category,Penetration} , TTS{TTS,BBT,BBT-Place,BBT-Place on invoice,BBT-Place off invoice,BBT-Price,CPP on invoice,CPP off invoice,BBT-Product,BBT-Pack,BBT-Proposition,BBT-Promotion,EOT}
...
#ensure the values are not in scientific notation and initiate the script time
#oad the required packages into the r script , if the package is not present in the r-env it will install aand load the package into the environment
...
########initiate the imapping path and the directory path for the script and load the input files for the ID-Bango GT Model #######

input.path <- "C:/Users/Gourab.Chanda/Desktop/Yashmin/Extracted_Data/Extracted_xlsx"
input.path.tts <- "C:/Users/Gourab.Chanda/Desktop/Yashmin/Extracted_Data/Extracted_xlsx/TTS_xlsx"
directory.path <- "C:/Users/Gourab.Chanda/Desktop/Yashmin"
mapping.path <- "C:/Users/Gourab.Chanda/Desktop/Yashmin/Extracted_Data/Mapping file/Jarvis ID- Universal Mapping.xlsx"
...
banner.mapping[which((banner.mapping$`Banner Desc`=="Not assigned")),"Banner code"] <- "#"
rownames(banner.mapping) <- NULL
...
subbrand.mapping <-  tibble::as_tibble(read_xlsx(paste0(mapping.path),sheet = "Sub brands",col_names = T,guess_max = 100))
...
packgroup_bmi_replication <- function(input_packgroup_region){
  packgroup.list <- c("Bango Manis Big Pouch","Bango Manis Big Sachet","Bango Manis Medium Bottle","Bango Manis Medium Pouch","Bango Manis Small Bottle","Bango Manis Small Pouch")
  for (i in packgroup.list ) {
    output_packgroup_region_i <- dplyr::mutate(input_packgroup_region,Required_Packgroup=i)
    return(output_packgroup_region_i)
  }
}
...
sellout_preprocessing <- function(input_sellout_data1,input_sellout_data2,mapping_set1=month.mapping,mapping_Set=sellout.customer.mapping){
  ...
  output_sellout_data_mapped%<>%convert(num(Sellout_Value))
...
#store the psales path into the id.sales.input.path variable in the script 
...
if(length(bango.ps.npm)>0){
  write.csv(bango.ps.npm,file = paste0(directory.path,"/psmaterial_unmapped.csv"),row.names = F)
}
if(length(bango.ps.ncm)>0){
  write.csv(bango.ps.ncm,file = paste0(directory.path,"/pscustomer_unmapped.csv"),row.names = F)
}
if(length(bango.ps.nconvm)>0){
  write.csv(bango.ps.nconvm,file = paste0(directory.path,"/psconversion_unmapped.csv"),row.names = F)
}
...
if(length(bango.ss.npm)>0){
  write.csv(bango.ss.npm,file = paste0(directory.path,"/ssmaterial_unmapped.csv"),row.names = F)
}
if(length(bango.ss.ncm)>0){
  write.csv(bango.ss.ncm,file = paste0(directory.path,"/sscustomer_unmapped.csv"),row.names = F)
}
if(length(bango.ss.nconvm)>0){
  write.csv(bango.ss.nconvm,file = paste0(directory.path,"/ssconversion_unmapped.csv"),row.names = F)
}
############################################################## (SStock) #################################################################
...
if(length(bango.sstock.npm)>0){
  write.csv(bango.sstock.npm,file = paste0(directory.path,"/sstock_material_unmapped.csv"),row.names = F)
}
if(length(bango.sstock.ncm)>0){
  write.csv(bango.sstock.ncm,file = paste0(directory.path,"/sstock_customer_unmapped.csv"),row.names = F)
}
if(length(bango.sstock.nconvm)>0){
  write.csv(bango.sstock.nconvm,file = paste0(directory.path,"/sstock_conversion_unmapped.csv"),row.names = F)
}
...
if(length(bango.droo.npm)>0){
  write.csv(bango.droo.npm,file = paste0(directory.path,"/droo_material_unmapped.csv"),row.names = F)
}
if(length(bango.sstock.ncm)>0){
  write.csv(bango.droo.ncm,file = paste0(directory.path,"/droo_customer_unmapped.csv"),row.names = F)
}
if(length(bango.droo.nconvm)>0){
  write.csv(bango.droo.nconvm,file = paste0(directory.path,"/droo_conversion_unmapped.csv"),row.names = F)
}
#perform the mapping for the observations from the extracted files with the product , customer and conversion mapping 
...
if(length(itrust.bango.npm)>0){
  write.csv(itrust.bango.npm,file = paste0(directory.path,"/itrustmaterial_unmapped.csv"),row.names = F)
}
if(length(itrust.bango.ncm)>0){
  write.csv(itrust.bango.ncm,file = paste0(directory.path,"/itrustcustomer_unmapped.csv"),row.names = F)
}
...
write.csv(x = x,file = paste0(directory.path,"/unmapped.tts.nochannel.csv"))
...
script.time <- (end.time-start.time)
print(script.time)